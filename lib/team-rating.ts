// AI Team Rating & Analysis Engine
...
/**
 * Calculates batting strength based on batsmen count and value
 */
export function calculateBattingScore(team: Team): number {
...
/**
 * Calculates team balance (proper role distribution)
 */
export function calculateBalanceScore(team: Team): number {
  const roleCount = {
  ...
  Object.entries(idealDistribution).forEach(([role, ideal]) => {
    const actual = roleCount[role as keyof typeof roleCount]
...
/**
 * Calculates value for money (spending efficiency)
 */
export function calculateValueScore(team: Team): number {
  if (team.players.length === 0) return 0

  const averagePrice = team.players.reduce((acc, p) => acc + p.basePrice, 0) / team.players.length
  const qualityRatio = team.players.filter((p) => p.basePrice > 50).length / team.players.length

  // Ideal average price is 50-60L
  const priceEfficiency = 1 - Math.abs(averagePrice - 55) / 55
  const qualityBonus = qualityRatio * 3 // Premium players boost value
...
/**
 * Generates insights about team strengths
 */
export function generateStrengths(
  team: Team,
  scores: { batting: number; bowling: number; balance: number; value: number },
): string[] {
...
/**
 * Generates insights about team weaknesses
 */
...
/**
 * Generates AI recommendations for team improvement
 */
export function generateRecommendations(
  ...
  if (scores.batting < 6) {
    recommendations.push("Focus on acquiring quality batsmen to strengthen the middle order")
  }

  if (scores.bowling < 6) {
    recommendations.push("Invest in experienced fast bowlers for death overs defense")
  }

  if (scores.value < 6) {
    recommendations.push("Better budget management - look for underrated talents in next auction")
  }

  const allrounders = team.players.filter((p) => p.role === "All-rounder")
  if (allrounders.length < 2) {
    recommendations.push("Add more all-rounders for flexibility and depth")
  }

  if (team.budget > 100) {
    recommendations.push("Significant budget remaining - consider upgrading bench strength")
  }
...
/**
 * Main function to calculate comprehensive team rating
 */
...
/**
 * Generates analytical insights about team composition
 */
function generateInsights(
  ...
  const battingPercentage =
    (team.players.filter((p) => p.role === "Batsman").length / Math.max(team.players.length, 1)) * 100
  const bowlingPercentage =
    (team.players.filter((p) => p.role === "Bowler").length / Math.max(team.players.length, 1)) * 100

  if (battingPercentage > 50) {
    insights.push("Aggressive team composition focused on batting dominance")
  } else if (bowlingPercentage > 45) {
    insights.push("Defensive strategy with emphasis on bowling control")
  }

  const totalValue = team.players.reduce((acc, p) => acc + p.basePrice, 0)
  const overallAvg = totalValue / Math.max(team.players.length, 1)

  if (overallAvg > 60) {
    insights.push("Star-studded lineup with expensive marquee players")
  } else if (overallAvg < 30) {
    insights.push("Young and hungry squad with budget-friendly selections")
  }

  if (team.players.length >= 25) {
    insights.push("Full squad utilization - complete retention strategy")
  } else if (team.players.length < 15) {
    insights.push("Selective approach - quality over quantity")
  }
...
/**
 * Compares two teams and generates comparative analysis
 */
export function compareTeams(team1: Team, team2: Team): { winner: string; analysis: string[] } {
  const rating1 = calculateTeamRating(team1)
  const rating2 = calculateTeamRating(team2)

  const analysis: string[] = []

  if (rating1.battingScore > rating2.battingScore + 1) {
    analysis.push(`${team1.name} has stronger batting prowess`)
  } else if (rating2.battingScore > rating1.battingScore + 1) {
    analysis.push(`${team2.name} has stronger batting prowess`)
  }

  if (rating1.bowlingScore > rating2.bowlingScore + 1) {
    analysis.push(`${team1.name} has a superior bowling attack`)
  } else if (rating2.bowlingScore > rating1.bowlingScore + 1) {
    analysis.push(`${team2.name} has a superior bowling attack`)
  }

  return {
    winner: rating1.overallScore > rating2.overallScore ? team1.id : team2.id,
    analysis,
  }
}
