...
function createPlayers() {
  ...
  return shuffled.map((player, i) => ({
    ...
    name: player.name,
    role: player.role,
    basePrice: player.basePrice, // Keep original base prices for proper marquee categorization
    previousTeam: player.team, // For reference
    stats: {
      matches: Math.floor(Math.random() * 150) + 50,
      runs: player.role.includes('Batsman') || player.role.includes('All-rounder') || player.role.includes('Wicket-keeper') ? Math.floor(Math.random() * 5000) + 1000 : Math.floor(Math.random() * 500),
      wickets: player.role.includes('Bowler') || player.role.includes('All-rounder') ? Math.floor(Math.random() * 200) + 20 : Math.floor(Math.random() * 10),
      average: player.role.includes('Batsman') || player.role.includes('All-rounder') || player.role.includes('Wicket-keeper') ? (Math.random() * 20 + 25).toFixed(2) : (Math.random() * 10 + 15).toFixed(2),
      strikeRate: player.role.includes('Batsman') || player.role.includes('All-rounder') || player.role.includes('Wicket-keeper') ? (Math.random() * 40 + 120).toFixed(2) : (Math.random() * 30 + 100).toFixed(2),
      economy: player.role.includes('Bowler') || player.role.includes('All-rounder') ? (Math.random() * 3 + 6).toFixed(2) : (Math.random() * 2 + 8).toFixed(2)
    },
    rating: Math.floor(player.basePrice * 6.5) + 50 // Rating based on base price
  }));
}
